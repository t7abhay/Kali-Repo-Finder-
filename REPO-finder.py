    ...
    def run(self):
        try:
            response = requests.get(
                self.schema+self.url, headers=headers).status_code
            if response == 200:
...
class ping_thread(threading.Thread):
    ...
    def run(self):
        p = subprocess.Popen(['ping', '-c 3', self.hostname],
        ...
        if not p[0].strip():
            ...
            print("\t    " + p[1].strip())
            response = ask(
                "\t   Stuck at finding mirror latency. Do you want to retry[y] or skip[n]?", 'n')
            if response:
                run(self.hostname)
        else:
            try:
                if "100% packet loss" in p[0].strip():
                    average = "[!] Unable to check " + self.hostname + \
                        " latency, potentially host block ICMP request."
                else:
                    average = p[0].strip().splitlines(
                    )[-1].split('=')[1].split('/')[1]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Kali Mirrorscripts-v2 by IceM4nn automatically select the best kali mirror server and apply the configuration')
    ...
    if https:
        ...
        try:
            ...
            print("[!] The package \"" + package +
                  "\" could not found in local apt cache. You may need to install it manually later after you've done update kali.")
            print("    For the time being, re-run the script without https support.")
            sys.exit(1)

    print("[+] Getting mirror list ...")
    response = requests.get(
        'https://http.kali.org/README.mirrorlist', headers=headers).text
    ...
    subprocess.Popen(temp % line, shell=True,
